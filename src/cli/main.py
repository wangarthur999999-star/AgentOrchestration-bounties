...
def cli():
    ...
    parser.add_argument(
        ...
        default="table",
        help="Output mode (json, table, text)",
    )
    ...
    if args.command == "init":
        print(f"Initializing project: {args.name} (output: {args.output})")
    elif args.command == "deploy":
        print(f"Deploying agent from manifest: {args.manifest} (output: {args.output})")
    elif args.command == "status":
        print(f"Checking agent status... (output: {args.output})")
    elif args.command == "logs":
        print(f"Fetching logs for agent: {args.agent_id} (output: {args.output})")
    else:
        parser.print_help()
        sys.exit(1)
