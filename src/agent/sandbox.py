...
class AgentSandbox:
    ...
    def safe_child_path(self, agent_id: str, child: str) -> Path:
        root = self._sandboxes.get(agent_id)
        if root is None:
            raise ValueError(f"no sandbox for agent {agent_id}")
        resolved = (root / child).resolve()
        if not str(resolved).startswith(str(root.resolve()) + os.sep) and resolved != root.resolve():
            raise ValueError(f"path traversal blocked: {child!r} resolves outside sandbox")
        return resolved
